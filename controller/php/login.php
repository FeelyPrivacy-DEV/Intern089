...
if(isset($_POST['manager_login'])) {
        ...
        if(password_verify( $pass, $record['password'])) {
            if($record['approved'] == true) {
                if($record['login_able'] == true) {
                    ...
                    header('location: http://test.feelyprivacy.com/s/view/d/index');
                    exit();
                }
                else {
    ...
    if($record) {
        if(password_verify( $pass, $record['password'])) {
            ...
            header('location: http://test.feelyprivacy.com/s/view/p/index');
            exit();
        }
        else {
            header('location: http://test.feelyprivacy.com/s/p?auth=failed');
        }
    }
    else {
        header('location: http://test.feelyprivacy.com/s/p?auth=failed');
