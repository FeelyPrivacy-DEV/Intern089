...
$(document).on('click', '#doc_forgot', function() {
    ...
    let doc_forgot = true;
    var xhr = new XMLHttpRequest();
    var url = "http://143.244.139.242/s/controller/php/forgot_password.php";
            ...
            xhr.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    if(xhr.responseText == 'true') {
                        window.location.href = 'http://143.244.139.242/s/p';
                    }
                    else if(xhr.responseText == 'notSent') {
                        $('#for_warn_pat').html('<p class="text-danger">For some reason emailnot sent (email error)</p>');
                        $('.reset_form').show();
    ...
    if(new_pass != '' && conf_pass != '') {
        if(new_pass == conf_pass) {
            ...
            xhr.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    if(xhr.responseText == 'true') {
                        window.location.href = 'http://143.244.139.242/s/';
                    }
                    else if(xhr.responseText == 'notSent') {
